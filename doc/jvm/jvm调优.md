<!-- TOC -->autoauto- [内存模型](#内存模型)auto    - [1、运行时数据区](#1运行时数据区)auto        - [1.1 程序计数器](#11-程序计数器)auto        - [1.2 Java虚拟机栈](#12-java虚拟机栈)auto        - [1.3 本地方法栈](#13-本地方法栈)auto        - [1.4 Java堆](#14-java堆)auto        - [1.5 方法区（也有人叫永久代）](#15-方法区也有人叫永久代)auto        - [1.6 常量池](#16-常量池)auto    - [2、对象的创建](#2对象的创建)auto        - [2.1 当虚拟机遇到一条New指令时：会进行如下步骤](#21-当虚拟机遇到一条new指令时会进行如下步骤)auto        - [2.2 两种内存分配的方式](#22-两种内存分配的方式)auto        - [2.3 如何保证分配内存时线程的安全性？](#23-如何保证分配内存时线程的安全性)auto    - [3、 对象的内存布局](#3-对象的内存布局)auto        - [3.1 对象头包括两部分信息](#31-对象头包括两部分信息)auto        - [3.2 实例数据](#32-实例数据)auto        - [3.3 对齐填充](#33-对齐填充)auto    - [4、 对象的访问定位](#4-对象的访问定位)auto        - [4.1 对象的访问方式](#41-对象的访问方式)auto            - [4.1.1 句柄访问](#411-句柄访问)auto            - [4.1.2 指针访问](#412-指针访问)auto- [基础篇](#基础篇)auto    - [1、回收算法](#1回收算法)auto        - [1.1 引用计数算法](#11-引用计数算法)auto        - [1.2 可达性分析算法](#12-可达性分析算法)auto    - [2、引用](#2引用)auto    - [3、判断一个对象生存还是死亡](#3判断一个对象生存还是死亡)auto        - [3.1 第一次标记](#31-第一次标记)auto        - [3.2 第二次标记](#32-第二次标记)auto        - [3.3 finalize() 方法](#33-finalize-方法)auto    - [4、回收方法区](#4回收方法区)auto        - [4.1 回收废弃常量](#41-回收废弃常量)auto        - [4.2 回收无用的类](#42-回收无用的类)auto- [算法篇](#算法篇)auto    - [一：标记-清除算法](#一标记-清除算法)auto        - [1.标记](#1标记)auto        - [2.清除](#2清除)auto    - [二：复制算法](#二复制算法)auto        - [1 HotSpot虚拟机的改良算法](#1-hotspot虚拟机的改良算法)auto            - [弱代理论](#弱代理论)auto        - [2 Hotspot虚拟机新生代内存布局及算法](#2-hotspot虚拟机新生代内存布局及算法)auto            - [分配担保](#分配担保)auto    - [三：标记-整理算法](#三标记-整理算法)auto        - [1 标记](#1-标记)auto        - [2 整理](#2-整理)auto    - [四：分代收集算法](#四分代收集算法)auto        - [1、为什么要对堆内存分代](#1为什么要对堆内存分代)auto        - [2、年轻代](#2年轻代)auto            - [复制整理算法](#复制整理算法)auto        - [3、老年代](#3老年代)auto        - [4、有关年轻代的配置参数](#4有关年轻代的配置参数)auto- [终结篇](#终结篇)auto        - [几个相关概念：](#几个相关概念)auto    - [一：Serial 收集器](#一serial-收集器)auto    - [二：ParNew收集器](#二parnew收集器)auto    - [三：Parallel Scavenge 收集器](#三parallel-scavenge-收集器)auto    - [四：Serial Old 收集器](#四serial-old-收集器)auto    - [五：Parallel Old 收集器](#五parallel-old-收集器)auto    - [六：CMS收集器](#六cms收集器)auto    - [七：G1收集器](#七g1收集器)auto- [调优篇](#调优篇)auto    - [一：调优步骤](#一调优步骤)auto        - [1.  监控GC的状态](#1--监控gc的状态)auto        - [2. 生成堆的dump文件](#2-生成堆的dump文件)auto        - [3. 分析dump文件](#3-分析dump文件)auto        - [4. 分析结果，判断是否需要优化](#4-分析结果判断是否需要优化)auto        - [5. 调整GC类型和内存分配](#5-调整gc类型和内存分配)auto        - [6. 不断分析和调整](#6-不断分析和调整)auto        - [7. 调优实例](#7-调优实例)auto    - [二：JVM调优参数参考](#二jvm调优参数参考)auto    - [二：堆大小设置](#二堆大小设置)auto    - [三：回收器选择](#三回收器选择)auto        - [1. 吞吐量优先的并行收集器](#1-吞吐量优先的并行收集器)auto        - [2. 响应时间优先的并发收集器](#2-响应时间优先的并发收集器)auto        - [3. 回收器搭配方案](#3-回收器搭配方案)auto            - [1）年轻代收集器](#1年轻代收集器)auto            - [2）老年代收集器](#2老年代收集器)autoauto<!-- /TOC -->
# 内存模型
