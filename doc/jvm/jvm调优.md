...
- [内存模型](#内存模型)
    - [1 、运行时数据区](#1-运行时数据区)
        ...
        - [1.3 本地方法栈](#13-本地方法栈)
        - [1.4 Java堆](#14-java堆)
        - [1.5 方法区（也有人叫永久代）](#15-方法区也有人叫永久代)
        - [1.6 常量池](#16-常量池)
    - [2 、对象的创建](#2-对象的创建)
        - [2.1 当虚拟机遇到一条New指令时：会进行如下步骤](#21-当虚拟机遇到一条new指令时会进行如下步骤)
        - [2.2 两种内存分配的方式](#22-两种内存分配的方式)
        - [2.3 如何保证分配内存时线程的安全性？](#23-如何保证分配内存时线程的安全性)
    - [3 、 对象的内存布局](#3--对象的内存布局)
        - [3.1 对象头包括两部分信息](#31-对象头包括两部分信息)
        - [3.2 实例数据](#32-实例数据)
        - [3.3 对齐填充](#33-对齐填充)
    - [4 、 对象的访问定位](#4--对象的访问定位)
        - [4.1 对象的访问方式](#41-对象的访问方式)
            - [4.1.1 句柄访问](#411-句柄访问)
            - [4.1.2 指针访问](#412-指针访问)
- [基础篇](#基础篇)
    - [1 、回收算法](#1-回收算法)
    ...
    - [2 、引用](#2-引用)
    - [3 、判断一个对象生存还是死亡](#3-判断一个对象生存还是死亡)
        - [3.1 第一次标记](#31-第一次标记)
        - [3.2 第二次标记](#32-第二次标记)
        - [3.3 finalize() 方法](#33-finalize-方法)
    - [4 、回收方法区](#4-回收方法区)
        - [4.1 回收废弃常量](#41-回收废弃常量)
        - [4.2 回收无用的类](#42-回收无用的类)
- [算法篇](#算法篇)
    - [一：标记-清除算法](#一标记-清除算法)
        - [1. 标记](#1-标记)
        - [2. 清除](#2-清除)
    - [二：复制算法](#二复制算法)
        - [1 HotSpot虚拟机的改良算法](#1-hotspot虚拟机的改良算法)
            - [弱代理论](#弱代理论)
        - [2 Hotspot虚拟机新生代内存布局及算法](#2-hotspot虚拟机新生代内存布局及算法)
            - [分配担保](#分配担保)
    - [三：标记-整理算法](#三标记-整理算法)
        - [1 标记](#1-标记)
        - [2 整理](#2-整理)
    - [四：分代收集算法](#四分代收集算法)
        - [1 、为什么要对堆内存分代](#1-为什么要对堆内存分代)
        - [2 、年轻代](#2-年轻代)
            - [复制整理算法](#复制整理算法)
        - [3 、老年代](#3-老年代)
        - [4 、有关年轻代的配置参数](#4-有关年轻代的配置参数)
- [终结篇](#终结篇)
        - [几个相关概念：](#几个相关概念)
    - [一：Serial 收集器](#一serial-收集器)
    - [二：ParNew收集器](#二parnew收集器)
    - [三：Parallel Scavenge 收集器](#三parallel-scavenge-收集器)
    - [四：Serial Old 收集器](#四serial-old-收集器)
    - [五：Parallel Old 收集器](#五parallel-old-收集器)
    - [六：CMS收集器](#六cms收集器)
    - [七：G1收集器](#七g1收集器)
- [调优篇](#调优篇)
    - [一：调优步骤](#一调优步骤)
        - [1. 监控GC的状态](#1-监控gc的状态)
        - [2. 生成堆的dump文件](#2-生成堆的dump文件)
        - [3. 分析dump文件](#3-分析dump文件)
        - [4. 分析结果，判断是否需要优化](#4-分析结果判断是否需要优化)
        - [5. 调整GC类型和内存分配](#5-调整gc类型和内存分配)
        - [6. 不断分析和调整](#6-不断分析和调整)
        - [7. 调优实例](#7-调优实例)
    - [二：JVM调优参数参考](#二jvm调优参数参考)
    - [二：堆大小设置](#二堆大小设置)
    - [三：回收器选择](#三回收器选择)
        - [1. 吞吐量优先的并行收集器](#1-吞吐量优先的并行收集器)
        - [2. 响应时间优先的并发收集器](#2-响应时间优先的并发收集器)
        - [3. 回收器搭配方案](#3-回收器搭配方案)
            - [1 ）年轻代收集器](#1-年轻代收集器)
            - [2 ）老年代收集器](#2-老年代收集器)
