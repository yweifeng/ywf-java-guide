...
- A写请求过来，将写请求放入blockingQueue中。后执行删除库存缓存操作，还未更新数据库。

- B读请求过来，发现blockingQueue中有未执行完成的操作，将读请求放入blockingQueue中，

  同时监测是否已经轮到自己的请求了。

- 此时A写请求成功更新数据库商品库存为999，从blockingQueue中移除A写请求。

- B读请求发现只有自己一个读请求了，执行读请求，读取数据库为999，更新缓存。



### 进一步优化

- 问题：假如对同一个有连续多个读请求，都进行堵塞，并没有多大的意义。
  - 解决方案：插入blockingQueue前判断是否是连续读请求。
- 问题：如果请求链比较长，读请求会长时间卡住 （用户体验度为200ms以内完成）
  - 解决方案：设置方法超时时间，如果方法超时，直接从数据库读取，并更新缓存。
